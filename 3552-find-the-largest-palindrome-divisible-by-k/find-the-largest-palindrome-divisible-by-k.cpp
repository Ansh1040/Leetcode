...
public:
    ...
    //     int halfSize = (n + 1) / 2;
    //     long long maxHalf = pow(10, halfSize) - 1;
    //     long long minHalf = pow(10, halfSize - 1);
        
    //     for (long long half = maxHalf; half >= minHalf; --half) {
    //         long long palindrome = constructPalindrome(half, n);
    //         if (palindrome % k == 0) {
    ...
    // long long constructPalindrome(long long half, int n) {
    //     bool evenLength = (n % 2 == 0);
    //     long long palindrome = half;

    //     if (!evenLength) {
    //         half /= 10;  // Skip the middle digit for odd-length palindromes
    //     }
